...
class map_shortcode {
	...
	$bubble1 = '[\'<a href=\"https://www.google.com/maps/dir/Current+Location/'.$lon1.','.$lat1.'\"><h5>'.$company_name.'</h5><span style=\"font-weight:bold;\">'.$main_street.'</span><br />'.$main_city.', '.$main_state.' '.$main_zip.'</a>\','.json_encode($lon1).', '.json_encode($lat1).']';
...
function ob_list_child_pages() { 

global $post; 

if ( is_page() && $post->post_parent ) 

	$childpages = 'parent'.wp_list_pages( 'sort_column=menu_order&title_li=&depth=1&child_of=' . $post->post_parent . '&echo=0' );
else
...
add_shortcode('child_pages', 'ob_list_child_pages');
